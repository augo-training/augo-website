...
interface NiceCoachesHeaderProps {
 ...
 *
 * cta_location is 'nice_coaches_header', not the athletes page's 'nice_header',
 * so the two funnels stay separable in Mixpanel.
 *
 * Padding comes from the page, which owns the single-screen layout.
 */
export default function NiceCoachesHeader({ unlocked }: NiceCoachesHeaderProps) {
        ...
        <header className="relative z-10 w-full max-w-[900px] mx-auto flex">
            {unlocked ? (
                <a
                    ...
                    onClick={() => trackCtaClicked({ cta_text: 'logo', cta_location: 'nice_coaches_header', destination: home })}
                >
                    {logo}
                </a>
            ) : (
