...
interface CoachCourseHeaderProps {
 ...
 *
 * cta_location is 'coach_course_header', so this funnel stays separable in
 * Mixpanel from the Nice pages, which share this header's shape.
 *
 * Padding comes from the page.
 */
export default function CoachCourseHeader({ unlocked }: CoachCourseHeaderProps) {
        ...
        <header className="relative z-10 w-full max-w-[900px] mx-auto flex">
            {unlocked ? (
                <a
                    ...
                    onClick={() => trackCtaClicked({ cta_text: 'logo', cta_location: 'coach_course_header', destination: home })}
                >
                    {logo}
                </a>
            ) : (
